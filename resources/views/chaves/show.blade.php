    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="mt-3">
                ...
                <strong>Sala:</strong> {{ $chave->sala ? $chave->sala->nome : 'Sala não atribuída' }}<br>
                <strong>Está Locada?</strong> 
                {{ $chave->is_locado ? 'Sim' : 'Não' }}<br>
                @if($chave->is_locado)
                    <strong>Funcionário Responsável:</strong> {{ $chave->funcionario ? $chave->funcionario->nome : 'Não especificado' }}<br>
                @endif
            </div>
        </div>
    </div>
